...
describe('CallExpressionTester', () => {
  describe('options', () => {
    ...
    it('should consume functions options', () => {
      const tester = new CallExpressionTester({}, {
        'functions': ['customFunctionWithResolvedPaths', 'jest.mock', 'invalid.with.parts']
      });

      expect(tester.simpleFunctions).to.eql(['require', 'customFunctionWithResolvedPaths']);
  ...
  describe('path testing', () => {
    ...
    it('should accept import()', () => {
      const parsed = parseToBody('var test = import("~/my/import")', { 'plugins': [importSyntaxPlugin] });
      const path = { 'node': parsed[0].declarations[0].init };
